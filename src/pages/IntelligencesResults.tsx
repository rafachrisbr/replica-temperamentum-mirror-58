...
const MultipleIntelligencesResults = () => {
  ...
  // Adapter function to make results compatible with IntelligenceCard
  const adaptIntelligenceResult = (result: any): AdaptedIntelligenceResult => {
    // Map kinesthetic to bodily
    const mappedType = result.type === 'kinesthetic' ? 'bodily' : result.type;
    ...
    return {
      ...
      color: result.color,
      percentage: Math.round((result.score / totalMaxScore) * 100),
      icon: result.type === 'kinesthetic' ? 'move' : 
            result.type === 'spatial' ? 'box' :
            result.type === 'interpersonal' ? 'users' :
            result.type === 'intrapersonal' ? 'user' :
            result.type === 'linguistic' ? 'book' :
            result.type === 'logical' ? 'calculator' : 'music',
      strengths: [],
      careers: [],
            ...
            <Button
              ...
              onClick={() => {
                try {
                  ...
                  toast.error('Seu navegador não suporta a função de compartilhamento.');
                }
              }}
            >
